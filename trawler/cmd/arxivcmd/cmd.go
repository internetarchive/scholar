...
var Cmd = &cobra.Command{
	Use:   "arxiv",
	Short: "Work with the arXiv upstream",
}
...
var StartScheduleCmd = &cobra.Command{
	Use:   "start-scheduler",
	Short: "start the schedule for daily arXiv crawling",
...
var StartOneOffCmd = &cobra.Command{
	Use:   "one-off",
	Short: "Kick off a single crawl for one day's worth of arXiv metadata",
	RunE: func(cmd *cobra.Command, args []string) error {
		crawlInput := daily.DailyCrawlWorkflowInput{
			Day:            skDay,
			SourceOverride: sourceOverride,
...
var StartInternalWorkerCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		log.Println("starting arxiv internal worker")
...
var StartExternalWorkerCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		log.Println("starting arxiv external worker")
		return daily.StartWorker(daily.WorkerDetails{
			Access:   "external",
			Upstream: "arxiv",
		})
