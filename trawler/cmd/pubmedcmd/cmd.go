...
var Cmd = &cobra.Command{
	Use:   "pubmed",
	Short: "Work with the PubMed upstream",
}
...
func init() {
	StartOneOffCmd.Flags().IntVar(&skLimit, "limit", 9999, "cap number of metadata entries to pull from pubmed. Default: 9999")
...
var StartScheduleCmd = &cobra.Command{
	Use:   "start-scheduler",
	Short: "start the schedule for daily pubmed crawling",
...
var StartOneOffCmd = &cobra.Command{
	Use:   "one-off",
	Short: "Kick off a single crawl for one day's worth of pubmed metadata",
	RunE: func(cmd *cobra.Command, args []string) error {
		crawlInput := daily.DailyCrawlWorkflowInput{
			Day:            skDay,
			Limit:          skLimit,
...
var StartInternalWorkerCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		log.Println("starting pubmed internal worker")
...
var StartExternalWorkerCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		log.Println("starting pubmed external worker")
		return daily.StartWorker(daily.WorkerDetails{
			Access:   "external",
			Upstream: "pubmed",
		})
